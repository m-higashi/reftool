...
# --------------------------------------------------------------------------
# 目次の読み取り(利用者が「目次を読み取る」を押したときだけ動く)
...
_BOILERPLATE = re.compile(
    r"contents\s+lists?\s+available|journal\s+homepage|sciencedirect|table\s+of\s+contents\s+alert",
    re.IGNORECASE)
_TOC_MAX_LINES = 200
_TOC_MIN_LINES = 5


# スキャンPDFのOCR層は、文字としては成立するが意味をなさない断片になることが多い。
# 「意味のありそうな語」を含む行の割合で足切りする。
...
_GENERATED_OUTLINE = re.compile(
    r"^(?:スライド|slide|ページ|page|p|図|表)\s*[\d\-_.]+$|_(?:ページ|page)_\d+$", re.IGNORECASE)


def _outline_is_generated(titles: list[str]) -> bool:
    """ページ送りやスライド番号だけのしおりは目次として使わない。"""
    if not titles:
        return True
    junk = sum(1 for t in titles if _GENERATED_OUTLINE.match(t.strip()))
    if junk >= len(titles) * 0.6:
        return True
    # 数字だけが違う同じ文字列が並ぶ場合も自動生成とみなす
    shapes = {re.sub(r"\d+", "#", t.strip()) for t in titles}
    return len(shapes) <= max(1, len(titles) // 8)


def _readable_ratio(lines: list[str]) -> float:
    if not lines:
        return 0.0
    return sum(1 for ln in lines if _WORDISH.search(ln)) / len(lines)


def _toc_score(text: str) -> int:
    """目次らしさ。**「目次」等の見出しが無いページは目次とみなさない**。

    行末がページ番号で終わる行が並ぶだけの表や索引を拾ってしまうため、
    見出しの存在を必須にしている(実測で誤検出が多かった)。
    """
    if not text or not _TOC_HEAD.search(text[:600]):
        return 0
    # 英文誌の1ページ目に必ず載る決まり文句。「Contents」を含むが目次ではない
    if _BOILERPLATE.search(text[:600]):
        return 0
    return 5 + sum(1 for ln in text.splitlines() if _TOC_LINE.match(ln.strip()))


def _clean_toc_line(line: str) -> str:
    """ページ番号と点線(リーダー)を落とす。既存の手入力に合わせて見出しだけ残す。"""
    ln = tidy(line)
    if not ln:
        return ""
    m = _TOC_LINE.match(ln)
    return tidy(m.group(1)) if m else tidy(_TRAIL_NUM.sub("", ln))


def extract_toc(path: Path, head_pages: int = 8, tail_pages: int = 6) -> dict:
    """目次の候補を返す。{"source": "bookmark"|"text"|None, "text": str, "page": int|None}

    ①PDFのしおり(ブックマーク)があれば、それを階層つきで使う(最も正確)
    ②無ければ巻頭・巻末のページから目次らしいページを1枚選び、本文を整えて返す
    """
    from pypdf import PdfReader

    empty = {"source": None, "text": "", "page": None}

    # ① しおり
    items = extract_pdf_outline(path)
    if len(items) >= 3 and not _outline_is_generated([t for _, t in items]):
        base = min(d for d, _ in items)
        lines = []
        for depth, title in items[:_TOC_MAX_LINES]:
            t = tidy(title)
            if t and not looks_garbled(t) and not _SYMBOLY.match(t):
                lines.append("  " * max(0, depth - base) + t)
        if len(lines) >= 3 and _readable_ratio(lines) >= 0.6:
            return {"source": "bookmark", "text": "\n".join(lines), "page": None}
    ...
    try:
        ...
        return empty
    targets = list(range(min(head_pages, n))) + list(range(max(0, n - tail_pages), n))
    best_score, best_page, best_text = 0, None, ""
    for i in sorted(set(targets)):
        ...
        if looks_garbled(text):
            continue          # 化けたページから作ると読めない目次になる
        s = _toc_score(text)
        if s > best_score:
            best_score, best_page, best_text = s, i + 1, text
    if best_score < 5 or not best_text:
        return empty
    ...
    for raw in best_text.splitlines():
        ...
        if len(lines) >= _TOC_MAX_LINES:
            break
    # 読める語を含む行が少ないページは、OCR層が壊れているとみなして採らない
    if len(lines) < _TOC_MIN_LINES or _readable_ratio(lines) < 0.75:
        return empty
    return {"source": "text", "text": "\n".join(lines), "page": best_page}
